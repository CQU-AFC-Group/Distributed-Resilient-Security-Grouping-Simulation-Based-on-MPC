...
void PX4CtrlFSM::process(std::string uav_name)
{
	...
	case MANUAL_CTRL:
	{
		if (rc_data.enter_hover_mode) // Try to jump to AUTO_HOVER
		{
			...
			{
				ROS_ERROR("[%s] [px4ctrl] Reject AUTO_HOVER(L2). No odom!", uav_name.c_str());
				break;
			}
			if (cmd_is_received(now_time))
			{
				ROS_ERROR("[%s] [px4ctrl] Reject AUTO_HOVER(L2). You are sending commands before toggling into AUTO_HOVER, which is not allowed. Stop sending commands now!", uav_name.c_str());
				break;
			}
			if (odom_data.v.norm() > 3.0)
			{
				ROS_ERROR("[%s] [px4ctrl] Reject AUTO_HOVER(L2). Odom_Vel=%fm/s, which seems that the locolization module goes wrong!", uav_name.c_str(), odom_data.v.norm());
				break;
			}
			...
			ROS_INFO("\033[32m [%s] [px4ctrl] MANUAL_CTRL(L1) --> AUTO_HOVER(L2)\033[32m",uav_name.c_str());
		}
		else if (param.takeoff_land.enable && takeoff_land_data.triggered && takeoff_land_data.takeoff_land_cmd == quadrotor_msgs::TakeoffLand::TAKEOFF) // Try to jump to AUTO_TAKEOFF
		{
			if (!odom_is_received(now_time))
			{
				ROS_ERROR("[%s] [px4ctrl] Reject AUTO_TAKEOFF. No odom!", uav_name.c_str());
				break;
			}
			if (cmd_is_received(now_time))
			{
				ROS_ERROR("[%s] [px4ctrl] Reject AUTO_TAKEOFF. You are sending commands before toggling into AUTO_TAKEOFF, which is not allowed. Stop sending commands now!", uav_name.c_str());
				break;
			}
			if (odom_data.v.norm() > 0.1)
			{
				ROS_ERROR("[%s] [px4ctrl] Reject AUTO_TAKEOFF. Odom_Vel=%fm/s, non-static takeoff is not allowed!", uav_name.c_str(), odom_data.v.norm());
				break;
			}
			if (!get_landed())
			{
				ROS_ERROR("[%s] [px4ctrl] Reject AUTO_TAKEOFF. land detector says that the drone is not landed now!", uav_name.c_str());
				break;
			}
			if (rc_is_received(now_time)) // Check this only if RC is connected.
			{
				...
				{
					ROS_ERROR("[%s] [px4ctrl] Reject AUTO_TAKEOFF. If you have your RC connected, keep its switches at \"auto hover\" and \"command control\" states, and all sticks at the center, then takeoff again.", uav_name.c_str());
					while (ros::ok())
					{
						...
						if (rc_data.is_hover_mode && rc_data.is_command_mode && rc_data.check_centered())
						{
							ROS_INFO("\033[32m [%s] [px4ctrl] OK, you can takeoff again.\033[32m", uav_name.c_str());
							break;
						}
					}
					break;
			...
			ROS_INFO("\033[32m [%s] [px4ctrl] MANUAL_CTRL(L1) --> AUTO_TAKEOFF\033[32m", uav_name.c_str());
		}
		...
		if (rc_data.toggle_reboot) // Try to reboot. EKF2 based PX4 FCU requires reboot when its state estimator goes wrong.
		{
			...
			{
				ROS_ERROR("[%s] [px4ctrl] Reject reboot! Disarm the drone first!", uav_name.c_str());
				break;
			}
			reboot_FCU();
		}
	...
	case AUTO_HOVER:
	{
		if (!rc_data.is_hover_mode)
		{
			...
			ROS_WARN("[%s] [px4ctrl] AUTO_HOVER(L2) --> MANUAL_CTRL(L1)", uav_name.c_str());
		}
		else if (rc_data.is_command_mode && cmd_is_received(now_time))
		{
			if (state_data.current_state.mode == "OFFBOARD")
			{
				...
				//des = get_cmd_des();
				des = get_hover_des();// 读取当前des，维持cmd状态
				ROS_INFO("\033[32m [%s] [px4ctrl] AUTO_HOVER(L2) --> CMD_CTRL(L3)\033[32m", uav_name.c_str());
			}
		}
		else if (takeoff_land_data.triggered && takeoff_land_data.takeoff_land_cmd == quadrotor_msgs::TakeoffLand::LAND)
		{
			...
			ROS_INFO("\033[32m [%s] [px4ctrl] AUTO_HOVER(L2) --> AUTO_LAND\033[32m", uav_name.c_str());
		}
		else
		{
			set_hov_with_rc();
				...
				(takeoff_land.delay_trigger.first && now_time > takeoff_land.delay_trigger.second))
			{
				...
				ROS_INFO("\033[32m [%s] [px4ctrl] TRIGGER sent, allow user command.\033[32m", uav_name.c_str());
			}
	...
	case CMD_CTRL:
	{
		if (!rc_data.is_hover_mode) //这里因为里程计异常，会导致触发手动控制模式，因此删除了里程计数据判断条件
		{
			state = MANUAL_CTRL;
			toggle_offboard_mode(false);

			ROS_WARN("[%s] [px4ctrl] From CMD_CTRL(L3) to MANUAL_CTRL(L1)!",uav_name.c_str());
		}
		else if (!rc_data.is_command_mode || !cmd_is_received(now_time))
		{
			state = AUTO_HOVER;
			set_hov_with_odom();
			des = get_hover_des();
			ROS_INFO("[%s] [px4ctrl] From CMD_CTRL(L3) to AUTO_HOVER(L2)!", uav_name.c_str());
		}
		else
		{
			//des = get_cmd_des();
			des = get_hover_des();// 外部指令控制模式，用当前位置替代des
		}

		if (takeoff_land_data.triggered && takeoff_land_data.takeoff_land_cmd == quadrotor_msgs::TakeoffLand::LAND)
		{
			ROS_ERROR("[%s] [px4ctrl] Reject AUTO_LAND, which must be triggered in AUTO_HOVER. \
					Stop sending control commands for longer than %fs to let px4ctrl return to AUTO_HOVER first.",
				uav_name.c_str(), param.msg_timeout.cmd);
		}
	...
	case AUTO_TAKEOFF:
	{
		...
		else if (odom_data.p(2) >= (takeoff_land.start_pose(2) + param.takeoff_land.height)) // reach the desired height
		{
			state = AUTO_HOVER;
			set_hov_with_odom();
			ROS_INFO("\033[32m [%s] [px4ctrl] AUTO_TAKEOFF --> AUTO_HOVER(L2)\033[32m", uav_name.c_str());
	...
	case AUTO_LAND:
	{
		if (!rc_data.is_hover_mode || !odom_is_received(now_time))
		{
			...
			ROS_WARN("[%s] [px4ctrl] From AUTO_LAND to MANUAL_CTRL(L1)!", uav_name.c_str());
		}
		else if (!rc_data.is_command_mode)
		{
			...
			set_hov_with_odom();
			des = get_hover_des();
			ROS_INFO("[%s] [px4ctrl] From AUTO_LAND to AUTO_HOVER(L2)!",uav_name.c_str());
		}
		else if (!get_landed())
		{
			des = get_takeoff_land_des(-param.takeoff_land.speed);
		...
		else
		{
			...
			if (print_once_flag)
			{
				ROS_INFO("\033[32m [%s] [px4ctrl] Wait for abount 10s to let the drone arm.\033[32m", uav_name.c_str());
			...
			if (extended_state_data.current_extended_state.landed_state == mavros_msgs::ExtendedState::LANDED_STATE_ON_GROUND) // PX4 allows disarm after this
			{
				...
				if (now_time.toSec() - last_trial_time > 1.0)
				{
					if (toggle_arm_disarm(false)) // disarm
					{
						...
						ROS_INFO("\033[32m [%s] [px4ctrl] AUTO_LAND --> MANUAL_CTRL(L1)\033[32m", uav_name.c_str());
					}
	...
	else
	{
		...
		debug_pub.publish(debug_msg);// 在外部指令控制模式下，debug信息不再可信，需重新处理。
	}

	// STEP4: publish control commands to mavros
	if (!(state == CMD_CTRL && use_external_control))
	{
	...
	// STEP6: Clear flags beyound their lifetimepx4ctrl] AUTO_TAKEOFF
...
// 由通道7决定cmd状态，若通道7开，则为true；若通道7关，则为false
bool PX4CtrlFSM::cmd_is_received(const ros::Time& now_time)
{
    //bool cmd_received = (now_time - cmd_data.rcv_stamp).toSec() < param.msg_timeout.cmd;
    bool ch7_triggered = rc_data.channel7_cmd_trigger;  // 关联通道7

    //return ch7_triggered || cmd_received;  // 通道7触发或收到外部cmd
	return ch7_triggered;
}
